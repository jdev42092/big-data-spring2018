...
data_dir = "/Users/jaydev/Desktop/DUSP/11.S941/ps04_data/LC08_L1TP_012031_20140809_20170304_01_T1"
...
def tif2array(location):
    ...
    """
    tif = gdal.Open(location)
    band1 = tif.GetRasterBand(1)
    band1_array = band1.ReadAsArray()
    band1_array = band1_array.astype(np.float32)
    return band1_array

# ## Test tif2array
# band1_loc = os.path.join(data_dir, "LC08_L1TP_012031_20140809_20170304_01_T1_B1.tif")
# band1_array = tif2array(band1_loc)
# band1_array.sum()
...
# ## Test retrieve_meta
# meta_loc = os.path.join(data_dir, "LC08_L1TP_012031_20140809_20170304_01_T1_MTL.txt")
# var_list = retrieve_meta(meta_loc)
# print(var_list)
...
def lst_calc(location):
    ...
    """
    # Define red, nir, and tirs arrays
    file_prefix = location.split('/')[-1]
    red_path = location + "/" + file_prefix + "_B4.TIF"
    nir_path = location + "/" + file_prefix + "_B5.TIF"
    tirs_path = location + "/" + file_prefix + "_B10.TIF"
    ...
    # Define constants from metadata
    mtl = location + "/" + file_prefix + "_MTL.txt"
    meta_constants = retrieve_meta(mtl)
    ...
    rad = rad_calc(tirs, meta_constants)
    bt = bt_calc(rad, meta_constants)
    pv = pv_calc(ndvi, 0.2, 0.5)
    emis = emissivity_calc(pv, ndvi)

    # Calculate land surface temperature and return it
...
bqa = tif2array(os.path.join(data_dir, 'LC08_L1TP_012031_20140809_20170304_01_T1_BQA.TIF'))
